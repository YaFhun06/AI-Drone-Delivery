...
class AuthService:
    ...
    def register(
        ...
        existing_user = (
            self.user_repository.find_by_email(
                normalized_email
            )
        )
        ...
        customer_role = (
            self.user_repository.find_role_by_name(
                "Customer"
            )
        )
    ...
    def forgot_password(self, email):
        ...
        print(
            f"[RESET PASSWORD] "
            f"Gửi tới {normalized_email}: "
            f"mã xác nhận là {reset_code}"
        )
    ...
    def reset_password(
        ...
        user = (
            self.user_repository.find_by_reset_token(
                token
            )
        )
    ...
    def get_profile(self, user_id):
        user = (
            self.user_repository.find_by_id(
                user_id
            )
        )
