...
class ETAService:
    ...
    def estimate(
        ...
        try:
            ...
            prompt = (
                f"Một drone giao hàng bay quãng đường "
                f"{result['distance_km']} km với tốc độ "
                f"{speed} km/h, thời gian ước tính là "
                f"{eta_minutes} phút. "
                f"Hãy viết 1 câu ngắn gọn, thân thiện "
                f"thông báo cho khách hàng về thời gian giao hàng này."
            )
            ...
            result["ai_explanation"] = (
                f"Dự kiến giao hàng trong khoảng "
                f"{eta_minutes} phút."
            )
